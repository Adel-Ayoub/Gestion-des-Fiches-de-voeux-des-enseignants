...
const ViewFichePage = ({ onBack }) => {
  ...
  const [module,setModule]= useState<Course>();
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {choices.map((choice, index) => {
              // Find the corresponding module for this choice
              if(semesterLabel=='S1'){
              const module : Course =moduless1.find(m => m.id == choice.moduleId);
              }else {
                const module : Course =moduless2.find(m => m.id == choice.moduleId);
              }
              console.log(moduless2,choice.moduleId);
              if (!module) {
                console.warn(`Module not found for choice ${choice.moduleId}`);
                return null;
              }

              const [level, specialty] = module.level.split('-');

              return (
                <tr key={choice.ficheId + '-' + choice.moduleId} className={index % 2 === 0 ? 'bg-white'
                  : 'bg-gray-50 print:bg-transparent'}>
                  <td className="border border-gray-300 px-4 py-3 text-center font-semibold">
                    <span
                      className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-indigo-100 text-indigo-700 print:bg-transparent print:border print:border-indigo-700">
                      {choice.rank}
                    </span>
                  </td>
                  <td className="border border-gray-300 px-4 py-3 font-mono text-sm">{module.code}</td>
                  <td className="border border-gray-300 px-4 py-3">
                    <div>
                      <div className="font-medium">{module.name}</div>
                      <div className="text-sm text-gray-600 mt-1">
                        CM: {choice.wantsCours ? 'Oui' : 'Non'} | TD: {choice.wantsTd}h | TP: {choice.wantsTp}h
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="border border-gray-300 px-4 py-3 text-center">
                    {/* You'll need to add groupCount to your choice or module data */}
                    {choice.groupCount || 'N/A'}
                  </td>
                </tr>
              );
            })}
          ...
          <tfoot>
            <tr className="bg-gray-100 print:bg-transparent font-semibold">
              ...
              <td className="border border-gray-300 px-4 py-3 text-center">
                {choices.reduce((sum, choice) => sum + (choice.groupCount || 0), 0)}
              </td>
            </tr>
          </tfoot>
        </table>
  ...
  const fetchFicheDetails = async () => {
    try {
      ...
      res.data.forEach(async (choice: fichechoice) => {
        ...
        const module = await axios.get(`http://localhost:8080/api/modules/${choice.moduleId}`, {
          headers: {
            'Authorization': localStorage.getItem('jwt')
          }
        });
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:8080/api/admin/export/fiche/" + ficheId + "?format=pdf", {
        headers:
          { 'Authorization': localStorage.getItem('jwt') }, responseType: 'blob'
      }).catch((error) => {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto p-8 print:p-0">
        <div className="bg-white shadow-lg print:shadow-none">
          {/* Document Header */}
          <div
            className="bg-indigo-600 text-white p-8 print:bg-white print:text-black print:border-b-4 print:border-black">
          ...
          <div className="p-8">
            ...
            <div className="mb-8 p-4 bg-gray-50 rounded-lg print:bg-transparent print:border print:border-gray-300">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center">
                  ...
                  <span className="ml-2 font-mono font-semibold">FV-{fiche.academicYear.replace('-',
                    '')}-{String(fiche.id).padStart(4, '0')}</span>
                </div>
              </div>
            </div>

            {/* Modules Table s1*/}
            {moduless1.length >0 && renderModuleTable(fiche.semester1Choices, 'S1')}
            {/* Modules Table s2*/}
            { moduless2.length>0 &&renderModuleTable(fiche.semester2Choices, 'S2')}
            ...
            <div className="mt-12 pt-8 border-t border-gray-300 text-center text-sm text-gray-600">
              <p>Document généré le {formatDate(new Date())} à {new Date().toLocaleTimeString('fr-FR', {
                hour:
